...
def add_comment(content, post_id):
    ...
    try:
        ...
        db.session.execute(text(sql), {"content":content, "author_id":author_id,
            "post_id":post_id, "date_added":date_added})
        db.session.commit()
    except:
        return False
    return True
...
def get_user_comments(id):
    sql = '''SELECT c.id, c.content, c.date_added, c.author_id, c.post_id,
        u.username FROM comment c JOIN "user" u ON c.author_id=u.id WHERE
        c.author_id=:id ORDER BY c.date_added DESC'''
    result = db.session.execute(text(sql), {"id":id})
    comments = result.fetchall()
    return comments
